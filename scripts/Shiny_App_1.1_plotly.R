...
library(glue)
library(leafpop)
library(plotly) # need for plotly
library(shinycssloaders) # fancy spinners for css
         ...
         tabPanel("Site Detection", 
           fluidRow(
            column(width = 7,        
                   ...
                   plotlyOutput('bar_graph', height = "50vh") %>% # Changed this to plotlyOutput
                     withSpinner(color="#0dc5c1")), # added loading spinner
...
#Detection Data ----    
  output$bar_graph <- renderPlotly({
    ...
    ggplotly(p) # turned the ggplot into a plotly figure
  })
